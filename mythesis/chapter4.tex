...
\section{Profile likelihood unfolding}
Profile likelihood unfolding (PLU) is a method in which the unfolding problem is translated into a 
profile likelihood fitting problem. In PLU, a profile likelihood fit is performed to estimate the 
POI which are then utilised to estimate the true distribution. As already discussed in \cref{sec:unfmethod},
unfolding related quantities are defined per-bin. It implies that profile likelihood estimators
are also required to be per-bin. In PLU, 

% One can construct a profile likelihood ratio as shown in \cref{eqn:likeliratio}. The numerator 
% is obtained by fixing a value for $\mu$ and finding the conditional-maximum likelihood estimator
% $\hat{\hat{\theta}}$ whereas the denominator is a global likelihood value obtained 
% from ML estimators $\hat{\mu}$ and $\hat{\theta}$. It is important to note that $\lambda(\mu)$
% is a function of the POI, however, its definition takes into account $\theta$. 


% \begin{equation}
%     \lambda (\mu) = \frac{L(x;\mu,\hat{\hat{\theta}})}{L(x;\hat{\mu},\hat{\theta})}
%     \label{eqn:likeliratio}
% \end{equation}



example of mu..signal strength NPs systematic uncertainties ... histfactory?